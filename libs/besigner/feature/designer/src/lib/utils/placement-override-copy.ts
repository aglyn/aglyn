...
/**
 * A prop value as words: declared-property tokens filled from the page's
 * values (or named, when the page has none), other tokens dropped, markup
 * stripped.
 */
function readableText(
  ...
  const filled = raw
    .replace(PROP_TOKEN, (_match, name: string) => {
...
function ownText(
  ...
  for (const key of TEXT_PROPS) {
    const text = readableText(props[key], propValues)
    if (text) return text
  }
  return ''
}
...
function firstDescendantText(
  ...
  for (const childId of childIds(nodes[id])) {
    ...
    seen.add(childId)
    const own = ownText(nodes[childId], propValues)
    if (own) return own
    const deeper = firstDescendantText(childId, nodes, propValues, seen)
...
/**
 * The picker's name for one part of a definition (AGL-3288): what it IS in
 * an author's words, and what it SHOWS.
 *
 ...
 *   `Text: "Design on a live canvas…"` for running text.
 * - A container is named by the first words inside it: `Card: Besigner`.
 * - An element with no words anywhere — an icon, an unlabeled image — is
 *   placed by what surrounds it: `Icon (in Besigner card)`.
 * - Failing all of that, the friendly type name alone.
 */
export function describePart(
  ...
  if (entry.isRoot) return placementCopy(kind).wholePart
  ...
  if (isAuthoredName(name, componentId)) return truncatePreview(name)

  const type = friendlyTypeName(componentId)
  const { propValues } = options

  const own = ownText(node, propValues)
  if (own) {
    const preview = truncatePreview(own)
    return QUOTED_TYPES.has(type) ? `${type}: "${preview}"` : `${type}: ${preview}`
  }

  const inner = firstDescendantText(id, nodes, propValues)
  if (inner) return `${type}: ${truncatePreview(inner)}`

  // Nothing to read inside: say where it sits instead. A plain group is a
  // wrapper an author never thinks of, so the nearest ancestor with a real
  // name ("card", "section") is preferred, and a group only when nothing
  // better encloses it.
  ...
  while (ancestor && !seen.has(ancestor)) {
    ...
    const words =
      ownText(ancestorNode, propValues) ||
      firstDescendantText(ancestor, nodes, propValues, new Set([id]))
    if (words) {
      const ancestorType = friendlyTypeName(ancestorNode?.componentId)
      const place = `${type} (in ${truncatePreview(words)} ${ancestorType.toLowerCase()})`
      if (ancestorType !== GROUP_TYPE) return place
      fallback ||= place
    }
