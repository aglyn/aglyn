 ...
 *      then escalates forever — and on a deployment with no AI provider
 *      key that reads as "Aglyn Assist is not configured",
 ...
 *
 * `docs-deflection.spec.ts` carries a fixed set of realistic questions —
 ...
 * the corpus moves. `docs-deflection.spec.ts` asserts the first one's band
 ...
 * list underneath.
 *
 * Three specs in `docs-deflection.spec.ts` hold the line — no emphasis
 ...
 * ## Why this exists
 *
 * A deployment with no AI provider key — which includes every self-hosted
