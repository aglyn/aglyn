...
export const CONSOLE_WIDGET_SLOTS = {
  ...
  /**
   * The commerce product editor, under the product's description, tags and
   * categories (AGL-2916). Props: {@link ConsoleProductEditorZoneProps} — the
   * product as the editor holds it, and `proposeValues`, which stages copy in
   * the editor as unsaved edits.
   *
   * Hosted by the commerce plugin through `useConsoleWidgetSlot`, as the
   * editor's search listing hosts `seoFields`. A widget here proposes and
   * never writes: Save product is the write, with the guards it carries.
   */
  productEditor: 'productEditor',
  /**
   * The commerce products hub, above its catalog table (AGL-2916). Props:
   * {@link ConsoleProductsHubZoneProps} — the catalog rows the hub holds, the
   * products its latest import created, and the hub's own writes a widget may
   * ask it to make: copy onto saved products, new draft products, categories
   * and switched-off discounts. Each is the hub's write, with the hub's seed,
   * allowance and slug checks; a widget writes nothing itself.
   */
  productsHub: 'productsHub',
  /**
   * Inside the commerce CSV import dialog (AGL-2916). Props:
   * {@link ConsoleProductImportZoneProps} — how many products the import
   * creates, and options a widget sets for what happens to them once they
   * land. The hub hands the options, with the ids of the products the import
   * created, to `productsHub` as its `lastImport`.
   */
...
/** What the `productEditor` zone hands each widget (AGL-2916). */
...
/**
 * What the `productsHub` zone hands each widget (AGL-2916). Every write
...
/** What the `productImport` zone hands each widget (AGL-2916). */
