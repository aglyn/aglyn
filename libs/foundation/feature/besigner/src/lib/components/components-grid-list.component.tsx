...
const ComponentsGridListComponent = forwardRef<any, ComponentsGridListProps>(function RefRenderFn(
  props,
  ref
) {
  ...
  const renderItemContent = useCallback(
    ({ icon, ...item }: AglynComponentElementTemplate) => (
      <CardIconListItem item={item} label={item.label} onActionClick={handleItemClick}>
        {!icon?.path && icon ? (
          (icon as any)
        ) : (
          <MdiIcon
