...
const BreadcrumbItem = forwardRef<any, BreadcrumbItemProps>(function RefRenderFn(props, ref) {
...
const AppBarBreadcrumbsComponent = forwardRef<any, AppBarBreadcrumbsComponentProps>(
      ...
      >
        <MuiToolbar variant="dense" sx={{ px: { xs: 2, sm: 2 } }}>
          <Stack
            ...
            spacing={1}
          >
            <Breadcrumbs separator="›" aria-label="breadcrumb" sx={{ lineHeight: 1, fontSize: 11 }}>
              {breadcrumbs.map(($id, index, arr) => (
                <BreadcrumbItem key={$id} $id={$id} isLast={index + 1 === arr.length} />
              ))}
            </Breadcrumbs>
            {children}
          </Stack>
