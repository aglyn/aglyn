...
/*
 * R3 OVER THE REAL TABLE lives with the table: `plan-entitlements-deliverable
 * .spec.ts` in `@aglyn/aglyn` checks every plan we sell against
 * `deliverableMonthlyCeiling`. Every R3 case in this file passes its allowance
 * in by hand, so it proves the model and nothing about what is sold. The core
 * may import this library and this library may not import the core, so the
 * check that needs both sits on the side that can see both.
 */
