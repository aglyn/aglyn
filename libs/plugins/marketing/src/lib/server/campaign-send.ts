...
export async function performCampaignSend(
  ...
  */
   ...
   *
   * Resolved ABOVE the dry run on purpose. `preview` is where a merchant
   * finds out what a send will do before writing copy, so it must answer the
   * same question a real send would — both which identity the mail leaves on,
   * and whether it may leave at all. Resolving after this point would let
   ...
   * never from `options`. A campaign's `From:` assembled from request input is
   * the spoofing path the verified-identity rule exists to close.
   *
