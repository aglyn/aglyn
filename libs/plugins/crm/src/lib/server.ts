 ...
 *
 * `crm/contact-stage` (AGL-2605) is the first real route, and it is a
 * server route for one reason: the EVENT. `contactStageChanged` can only be
 * emitted by a server path that performed the write, so a record page that
 * moved the stage with a client-direct write would move the person and
 * tell no automation. Every other field on the facet stays client-direct;
 * this one goes through here because something listens for it.
 */
