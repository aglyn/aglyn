...
import { DealTasksCard } from './deal-tasks-card'
            ...
            <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} sx={{ alignItems: 'stretch' }}>
              <Stack sx={{ flex: 1, minWidth: 0 }}>
                <DealPropertiesCard
                  deal={deal}
                  pipeline={pipeline}
                  ownerLabel={roster.labelFor(deal.ownerUid)}
                  routes={routes}
                  onEdit={() => setEditing(true)}
                />
              </Stack>
              <Stack sx={{ flex: 1, minWidth: 0 }}>
                <DealTasksCard hostId={hostId} dealId={deal.$id} />
              </Stack>
            </Stack>
            <DealActivityCard hostId={hostId} dealId={deal.$id} />
          </>
        ) : null}
      </Stack>
      <DealEditDrawer
