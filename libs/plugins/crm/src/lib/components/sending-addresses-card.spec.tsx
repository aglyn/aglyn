...
const mockAuthorizedFetch = jest.fn(async (_user: unknown, url: string, init: RequestInit = {}) => {
  ...
  if (method === 'GET') return json(200, { aliases: rows, signInEmail: 'avery@example.com' })
  if (method === 'DELETE') {
  ...
  switch (body?.['action']) {
    ...
    case 'confirm':
      rows = rows.map((row) =>
        row.address === 'avery@example.org' ? { ...row, verified: true, verifiedAtMs: 99 } : row,
      )
      return json(200, { ok: true, orgId: 'org-1', address: 'avery@example.org', alreadyConfirmed: false })
    default:
...
const mockUser = { uid: 'u-avery', getIdToken: async () => 'token' }
...
describe('SendingAddressesCard (AGL-2975)', () => {
  it('asks for the list only once the scope is ready, and says what the list is for', async () => {
    ...
    expect(
      await screen.findByText('You sign in as avery@example.com, which already counts as yours.'),
    ).toBeTruthy()
  })

  it('prints each address with its state: verification sent, or verified', async () => {
    rows = [
      { address: 'avery@example.org', addedAtMs: 1, verified: false, verifiedAtMs: null },
      { address: 'outreach@aglyn.io', addedAtMs: 2, verified: true, verifiedAtMs: 3 },
    ]
    render(<SendingAddressesCard orgId="org-1" ready />)
    await screen.findByText('avery@example.org')
    expect(within(rowOf('avery@example.org')).getByText(VERIFICATION_SENT_LABEL)).toBeTruthy()
    expect(
      within(rowOf('avery@example.org')).getByRole('button', {
        name: 'Resend the verification email to avery@example.org',
      }),
    ).toBeTruthy()
  ...
  it('adds an address with the page it was added on, then shows it waiting for verification', async () => {
    ...
    fireEvent.change(field, { target: { value: 'Avery@Example.org' } })
    ...
    expect(within(await waitFor(() => rowOf('avery@example.org'))).getByText(VERIFICATION_SENT_LABEL)).toBeTruthy()
  ...
  it('prints the route’s refusal and keeps what was typed', async () => {
    ...
    expect((field as HTMLInputElement).value).toBe('avery@example.com')
  })

  it('redeems a verification link once, takes the token out of the address bar, and shows the address verified', async () => {
    rows = [{ address: 'avery@example.org', addedAtMs: 1, verified: false, verifiedAtMs: null }]
    ...
    await waitFor(() =>
      expect(enqueueSnackbar).toHaveBeenCalledWith('avery@example.org verified', expect.anything()),
    )
    await waitFor(() => expect(within(rowOf('avery@example.org')).getByText(VERIFIED_LABEL)).toBeTruthy())
  ...
  it('removes a waiting address at once, and a verified one only after the confirmation', async () => {
    ...
    render(<SendingAddressesCard orgId="org-1" ready />)
    const removeWaiting = await screen.findByRole('button', { name: 'Remove avery@example.org' })
    await waitFor(() => expect((removeWaiting as HTMLButtonElement).disabled).toBe(false))
    fireEvent.click(removeWaiting)
    await waitFor(() => expect(screen.queryByText('avery@example.org')).toBeNull())
    ...
    expect(requests.filter((entry) => entry.method === 'DELETE').map((entry) => entry.body)).toEqual([
      { orgId: 'org-1', address: 'avery@example.org' },
