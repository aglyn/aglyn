...
import { listPluginEventHandlers, registerPluginEventHandler } from '@aglyn/aglyn/server'
 ...
 * have to be in place from the first request.
 *
...
export function registerAiServerDeclarations(): void {
  ...
  // Idempotent against the REGISTRY rather than a module flag, so a
  // registry reset (a spec) or a second module instance re-subscribes.
  if (listPluginEventHandlers('org.seatAddons.changed').includes(AI_PLUGIN_ID)) return
