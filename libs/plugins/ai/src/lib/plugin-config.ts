...
export const AI_STEP_MODEL_SETTING: Record<AiStepKind, string> = {
  ...
  'job.component': 'jobComponentModel',
  'job.crm': 'jobCrmModel',
  'job.experiment': 'jobExperimentModel',
  'job.form': 'jobFormModel',
  'job.insight': 'jobInsightModel',
  'job.layout': 'jobLayoutModel',
  'job.template': 'jobTemplateModel',
  'job.page': 'jobPageModel',
  'job.products': 'jobProductsModel',
  'job.seo': 'jobSeoModel',
...
export const AI_CONFIG_SCHEMA: PluginConfigSchema = {
  ...
  fields: [
    ...
    modelField('job.component', 'Generation job (component) model', 'Generating a reusable component and its properties.'),
    modelField('job.crm', 'Generation job (CRM) model', 'CRM record summaries and next steps, email drafts and import column matches.'),
    modelField(
      'job.experiment',
      'Generation job (A/B test) model',
      'Writing variants for an A/B test, and explaining what a test’s result means.',
    ),
    modelField('job.form', 'Generation job (form) model', 'Generating a form from a confirmed plan.'),
    modelField('job.insight', 'Insights model', 'Answering a question about the site’s figures, and the weekly insights.'),
    ...
    modelField('job.page', 'Generation job (page) model', 'Generating a page, one section at a time, from a confirmed plan.'),
    modelField('job.products', 'Generation job (products) model', 'Writing product copy, and proposing products, categories and discounts for a store.'),
