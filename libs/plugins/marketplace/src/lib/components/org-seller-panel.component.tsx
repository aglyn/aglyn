...
import { pluginDocsHelp, useMediaPicker } from '@aglyn/aglyn'
...
} from '../model/seller-ledger-totals'
...
export interface OrgSellerPanelProps {
   ...
   * shell (AGL-3080) — the panel's links to a listing and to the publish
   * form hang beneath it. It replaced an org slug and the console's route
   * table, which a plugin may not import.
   */
  basePath: string
}
...
export function OrgSellerPanel(props: OrgSellerPanelProps) {
  const { orgId, section, basePath } = props
   ...
   * provides `pickMedia` around every plugin page for exactly this. The URL
   * comes back already resolved — CDN path over the raw download URL
   * (AGL-1215) — so the panel no longer resolves one itself.
   */
  const { pickMedia } = useMediaPicker()
  const chooseLogo = useCallback(async () => {
    const picked = await pickMedia?.({ kind: 'image' })
    if (picked?.url) setAvatarUrl(picked.url)
  }, [pickMedia])
  ...
  const listingHref = (listing: any) => listingPath(basePath, listing.$id)
  ...
  const newVersionHref = (listing: any) =>
    publishPluginPath(basePath, listing.$id)
        ...
        <Stack spacing={2}>
          ...
          <Stack direction="row" spacing={2} sx={{ alignItems: 'center' }}>
            ...
            <Stack direction="row" spacing={1}>
              <Button size="small" onClick={chooseLogo}>
