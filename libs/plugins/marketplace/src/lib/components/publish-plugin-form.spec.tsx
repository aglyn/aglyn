...
// opening it would drag the whole media library into a form test. Since
// AGL-3080 it arrives through the console's context rather than as a dialog
// this form mounts, so there is nothing to render — only a door that stays
// shut when no console provides one.
...
describe('PublishPluginForm (AGL-969 / AGL-1076 / AGL-1078)', () => {
  const open = () => render(<PublishPluginForm orgId="org-1" basePath="/acme/marketplace" />)
  /**
  ...
  const openStrict = () =>
    render(
      <StrictMode>
        <PublishPluginForm orgId="org-1" basePath="/acme/marketplace" />
      </StrictMode>,
    )
