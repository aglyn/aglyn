...
export interface PublishPluginFormProps {
   ...
   * shell (AGL-3080). The listing the publish lands on hangs beneath it; it
   * replaced an org slug and the console's route table, which a plugin may
   * not import.
   */
  basePath: string
  /**
...
export function PublishPluginForm(props: PublishPluginFormProps) {
  const { orgId, basePath, listing, revocation } = props
   ...
   * provides `pickMedia` around every plugin page for exactly this.
   */
  const { pickMedia } = useMediaPicker()
  const insertReadmeImage = useCallback(async () => {
    const media = await pickMedia?.({ kind: 'image' })
    if (!media) return
    // A reference, not the origin-absolute CDN URL (AGL-1705) — the same
    // switch the listing detail editor's README body makes, and for the
    // reason established there: this markdown renders only in the console,
    // through the one `marketplaceListing` slot, so nothing was relying on
    // the baked-in origin. The URL stays the fallback for a free-tier asset,
    // which is minted no `cdnPath` and so has no reference to name.
    const src = formatMediaRef(media.mediaScope, media.mediaId) ?? media.url
    // The alt was a hardcoded '' (AGL-1896). Same reasoning as the listing
    // detail editor: no alt prompt exists on this surface, and a
    // markdown-lite image row's alt is fixed at insert time — so the asset's
    // own alt is the only description that could ever land here, and '' was
    // otherwise permanent.
    if (src)
  ...
  const submit = async () => {
    ...
    try {
      ...
      router.push(listingPath(basePath, String(payload?.listingId ?? '')))
    } catch (error) {
      console.error(error)
      ...
      >
        <Stack spacing={2}>
          ...
          <MarkdownField
            ...
            value={draft.readme}
            onChange={set('readme')}
            onPickImageFromMedia={insertReadmeImage}
            editorRef={(handle) => {
