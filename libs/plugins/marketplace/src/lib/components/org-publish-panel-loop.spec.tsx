...
const FIRESTORE = {}
// The two data hooks resolve from the query's path, so each picker gets its
// own list — the point is to exercise the real component, not a stubbed
// shell. They are named exports of the instance package since AGL-3080 moved
// this panel out of the console, where they were app-local re-export shims.
jest.mock('@aglyn/tenant-feature-instance', () => ({
  useFirestore: () => FIRESTORE,
  useUser: () => ({ data: { getIdToken: async () => 'token' } }),
  useFirestoreCollection: (factory: () => any) => {
    const ref = factory()
    const path: string = ref?.path ?? ''
    const key = Object.keys(firestoreCollections).find((k) => path.endsWith(k))
    return { data: key ? firestoreCollections[key] : [] }
  },
  useFirestoreDoc: () => ({ data: { handle: 'test-org', $id: 'org-1' } }),
}))
