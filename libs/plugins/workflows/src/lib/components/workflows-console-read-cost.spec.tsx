...
/**
 * What one WORKFLOWS console load actually READS (AGL-2501).
 *
...
/*
 * Collections only ONE section reads. `workflows` is deliberately NOT the
 * key for the Workflows section: the Webhooks card reads it too, to name the
 * workflow a hook fires. A collection two sections share cannot show that one
 * of them stayed closed — it is present either way — so the discriminating
 * collection is `actions`, which only the Actions card opens.
 */
...
function shellSections() {
  return WORKFLOWS_CONSOLE_SECTIONS.map((section) => ({
    id: section.id,
    label: section.label,
...
describe('workflows console read cost (AGL-2501)', () => {
  afterEach(() => {
    mockSection = ''
    mockListens.length = 0
  })
  ...
  it('CONTROL: the open section does listen, and for its own collection', async () => {
    ...
    expect(listenedCollections()).toContain('actions')
  })
  ...
  it('an unopened section issues no query', async () => {
    ...
    expect(seen).toContain('webhooks')
