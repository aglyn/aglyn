...
describe('which due enrollments run', () => {
  ...
  it('skips what is not due, not active, not this mailbox, or outside its window', () => {
    const result = select([
      ...
      candidate('weekend-only', 0, now - 1, {}, { settings: { window: { days: [0, 6], startMinute: 540, endMinute: 1020 }, allowedCountries: ['US'], allowCustomers: false } }),
  ...
  it("sends a sequence's own window when the mailbox's is closed", () => {
    ...
    const weekend = { window: { days: [6], startMinute: 540, endMinute: 1020 }, allowedCountries: ['US'], allowCustomers: false }
    const result = select(
