...
import { createStyles, WithStyles, withStyles, styled, Theme, emphasize, decomposeColor,recomposeColor  } from '@material-ui/core/styles'
...
export const CardIconListItem = withStyles(itemStyles, { name: 'IconListItem' })(forwardRef<any, ItemProps & WithStyles<typeof itemStyles>>(
  ...
  forwardRef<VirtuosoGridHandle, GridListProps & WithStyles<typeof styles>>(
    function RefRenderFn(props, ref) {
        ...
        function RefRenderFn(props, ref) {
          ...
          return (
            <ItemWrapper ref={ref} className={classes.itemWrapper} {...restProps}>
              <ItemContent className={classes.itemContent}>
                {children}
              </ItemContent>
            </ItemWrapper>
          )
      ...
      const itemContent = useCallback((index) => (
        <MemoizedItemContent>{renderItemContent(items[index], index, items)}</MemoizedItemContent>
      ), [renderItemContent])
