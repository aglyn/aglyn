...
export default function useTimeoutDelay(callback: Handler, options?: Options): TimeoutReturn {
  ...
  const start = useCallback((opt?: Options) => {
    ...
    if (false) {
      ...
      console.warn(
        'Can\'t start timeout or interval when one is already running', ref.current.timeoutRef)
    }
    ...
    const handler = (arg: HandlerParams['args']) => {
      ...
      if (limitReached || toCancel) {
        ...
        limitReached && _isFn(optOnLimitReach) && optOnLimitReach({
          startTime: Number(ref.current.startTime),
