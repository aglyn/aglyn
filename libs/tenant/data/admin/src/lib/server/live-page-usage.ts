 ...
 *
 * The content twin of the console's `screenIdsUsingFormDeep`, and the same
 * walk for the same reason. Saving, publishing or deleting an ENTRY changes
 * every page that renders the collection it belongs to, and only some of those
 * pages are derivable from the collection's slug. `/blog`, `/blog/my-post` and
