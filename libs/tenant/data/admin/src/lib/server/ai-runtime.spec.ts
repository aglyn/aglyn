...
// The module under test reaches `firebase-admin/firestore` only through
// `assist-usage`, and only for `FieldValue` statics the rate table never
// touches. Stubbed so the suite needs no app.
...
import { estimateAssistCostUsd } from './assist-usage'
import {
  AI_UPSTREAM_FAILURE_COPY,
  AiRequestShapeError,
  AiUpstreamError,
  aiFailureIsRetryable,
  buildAiRequestBody,
  runAiRequest,
  type AiStreamEvent,
} from './ai-runtime'
...
/**
 * The shared Anthropic runtime (AGL-2903).
 *
...
describe('a non-streaming request', () => {
  ...
  it('returns the text, the tool inputs as OBJECTS, the usage and the cost at the serving model’s rates', async () => {
    ...
    expect(result.estCostUsd).toBe(
      estimateAssistCostUsd(result.usage, 'claude-haiku-4-5'),
    )
    expect(result.estCostUsd).not.toBe(
      estimateAssistCostUsd(result.usage, 'claude-sonnet-5'),
    )
  })
  ...
  it('re-emits text deltas, assembles a tool block from its fragments, and closes with usage and cost', async () => {
    ...
    expect(events).toEqual([
      ...
      { type: 'tool', name: 'emit_section', input: { rootId: 'n1', nodes: {} } },
      {
        ...
        estCostUsd: estimateAssistCostUsd(
...
describe('the error boundary (AGL-2815)', () => {
  ...
  it.each([
    ...
    expect(aiFailureIsRetryable(status, type)).toBe(retryable)
  })
