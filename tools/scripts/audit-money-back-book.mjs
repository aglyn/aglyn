...
//
// WHY THE CLASSIFIERS ARE EXPORTED: a zero from an audit is worthless unless
// the query can be shown to match something. `audit-money-back-book.test.mjs`
// drives both classifiers over the pre-fix and post-fix object shapes, so a
// query that can never match anything fails the suite instead of reporting a
// reassuring zero. Keep that suite honest before trusting a zero from here.
//
...
async function auditBookings() {
  ...
  for (const s of sessions) {
    ...
    rows.push({
      ...
      paymentIntentId:
        pi?.id ?? (typeof s.payment_intent === 'string' ? s.payment_intent : null),
...
async function main() {
  ...
  const report = {
    ...
    bookings: { ...bookings, misrouted: misrouted.length, indeterminate: indeterminate.length, owedByHost },
  ...
  if (asJson) {
    ...
    for (const r of bookings.rows) {
      const tag =
        r.misrouted === true ? 'MISROUTED' : r.misrouted === null ? 'UNKNOWN  ' : 'ok       '
      console.log(
    ...
    console.log(`subscriptions scanned      : ${subscriptions.totalSubscriptionsScanned}`)
    ...
    console.log(`balance available   : ${report.corroboration.available.join(', ')}`)
