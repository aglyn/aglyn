...
export function mainVersionVerdict({ tagVersion, mainVersion }) {
  ...
  return {
    ...
    lines: [
      ...
      '  !!',
      '  !!    1. test:version-monotonic goes RED on main, in CI as well as',
      `  !!       locally ("package.json is ${mainVersion} but production runs`,
      `  !!       ${tagVersion}"). Main Gate fetches production and the tags,`,
      '  !!       so every push to main fails it and pings Slack #ci until',
      '  !!       main carries the bump.',
      ...
      '  !!',
      '  !!  The back-merge was owed the moment the promotion merged. Do it now,',
      '  !!  as a MERGE of production back into main, so the bump commit becomes',
      '  !!  an ancestor of main and the next PR is a clean fast-forward. From a',
