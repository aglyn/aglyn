...
| A person erased from a customer's CRM | **Until the next nightly run**, then permanent | `personErasures/{orgId}__{sha256(email)}`, filed by a workspace admin from the person's record (AGL-2623) and drained by `/api/admin/run-erasures` after the workspace batch. No hold: a person's request has no owner to reverse it and the runbook says not to carry the org model over. The sweep (`erase-person.ts`) deletes the contact on every site, leads, tasks, activities, audience memberships and the delivery log, and anonymizes orders and bookings; the completed request keeps counts and the hash, never the address. A per-site suppression row keyed by the hash outlives it so the address cannot be re-captured. | `erase-person.ts`, `run-person-erasures.ts` |
...
existed, the second half of this promise was reachable only by a human with
GCS console access — a retention commitment with no product path is a
commitment about a filing cabinet nobody has the key to.
...
reaches it — but it is keyed by the person, and an ACCOUNT erasure has to find
it too: `recursiveDelete(users/{uid})` cannot see a document under somebody
else's org. `eraseUser` therefore sweeps it explicitly — by path through every
current membership, then by a collection-group query on the `uid` field for
the months left behind in workspaces the person had already been removed from.
Each month expires on the thirteen-month window above regardless.
...
| 5 | ~~**`privacy@aglyn.com` may not exist.**~~ **CLOSED 2026-08-19 (AGL-1911).** All six published intakes — `privacy@`, `legal@`, `security@`, `abuse@`, `dmca@`, `support@` — are Google Groups on `aglyn.com`, each accepting unmoderated posts from anyone on the web and delivering every message to a single human recipient. Verified by reading Workspace group configuration, **not** by a test send: AGL-1577's default routing accepts and silently discards mail to non-existent `@aglyn.com` addresses, so a delivered probe would have been consistent with the mailbox never having existed. The Drive open-items list saying only `noreply@` and `info@` were live is stale. Residual: each group has one member and no auto-acknowledgement — **AGL-2400**. | **AGL-1911** |
