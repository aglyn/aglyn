...
| `backfill-intrinsic-media-size.mjs` | Outstanding | The width/height pair that keeps a published image from reflowing the page. `--apply` is refused unless `--project` matches the resolved credential. Proven by an emulator spec. |
