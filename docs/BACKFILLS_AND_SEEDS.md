...
| `backfill-account-acquisition.mjs` | ⚑ Outstanding | Stamps every account and workspace created before first-touch capture (AGL-3289) with `acquisition: { source: 'unknown' }`, marked `recordedBy: 'backfill'`, so the staff Acquisition card is never blank. Fills only what the platform can still say — the auth record's creation time and provider, and where the first device was last seen — and never a channel, referrer or campaign. Each write is a transaction that leaves a document alone if a record arrived since the plan, and accounts younger than the live window are skipped. Guarded by `test:account-acquisition-backfill`. Dry run by default; `--apply` writes. |
...
| `backfill-dataset-filter-keys.mjs` | ⚑ Outstanding, **then repeatable** | Stamps `filterKeys` — the token array the dataset records table filters and searches with `array-contains` — onto records written before the field. Every writer stamps it since the field shipped; a record without it answers no served filter. Re-run after a schema change moves a field's tokens (a text field given options, a field removed): the schema dialog does not rewrite records. Idempotent: a record whose stored array equals the computed one is not written. Carries a copy of the token builder (`lib/dataset-filter-keys.mjs`), held to the library by shared fixtures; `--self-test` checks those and the model shim. Dry run by default; `--org=<id>` narrows, `--apply` writes. |
