...
| Workspace subdomains | The in-console attach uses Vercel's project-domain API, so off Vercel it is skipped and org creation still succeeds — the console then advertises `{slug}.<NEXT_PUBLIC_WORKSPACE_DOMAIN>` as the workspace URL. **Point a wildcard DNS record for `*.<workspace domain>` at your console and route it there**, or that URL will not resolve. The skip is logged once per instance as `[workspace-domains] not configured`; it is a log rather than a refusal because failing org creation over a missing hosting-vendor credential would be worse than the gap. |
