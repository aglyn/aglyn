...
| AI generation | Same key, no new secret. The generative doors (sections, pages and automations written by a model) run on the same Anthropic runtime as Assist and are behind `release_ai_generative`, **off** by default in Remote Config and staff-preview only; every generative route answers 404 while it is off. During a provider incident the `ai-generate` feature key on **Staff → Lockdown** stops generation without touching Assist, which has its own `ai-assist` key. Generation shares the per-workspace message and cost ceilings above. |
...
| Sign-up attribution | **Nothing to set.** Where each account came from — its first visit's landing page, referrer, campaign tags and ad click ids, and the door it signed up through — is captured by the console and the site runtime themselves and written on the account at creation (AGL-3289). No analytics vendor is involved. The hosts it treats as yours are built from the addresses you already configure plus any added on **Staff → Platform settings**; cross-domain hops are sealed with the `TOKEN_SIGNING_SECRET` you already share; where an account was created from comes from the geo headers above. See `apps/docs/docs/staff-console/first-party-surfaces.md`. |
...
| Sequence link domains (AGL-3306) | Generic, and optional. A tracked sequence link reads `{NEXT_PUBLIC_CONSOLE_URL}/api/outreach/l/<id>` until an org sets up `links.<its mailbox domain>` under **Sequences → Mailboxes → Link domains** and it verifies. The attach goes through the domain driver at scope `console` (`vercel` attaches it to `VERCEL_CONSOLE_PROJECT_ID`, `webhook` sends your endpoint an `attach`); under `wildcard` or `none` you point the name at the console yourself — DNS plus a proxy route with a certificate that passes `Host` through. The console's middleware answers the host by `Host` on any deployment (`/<id>` → the short-link route, `/_aglyn/link-host` → the verification probe, everything else 404), and **Check** fetches the probe over HTTPS, so a link moves onto the host only once DNS, TLS and routing all work. A proxy or WAF that challenges non-browser clients must let `links.*` through on those two paths, or mail scanners get a challenge instead of the redirect. `AGLYN_LINK_HOST_TARGET` names the CNAME target the card prints. See `apps/docs/docs/developers/self-hosting-environment.md#sequences-link-domains`. |
