...
| field | meaning |
| --- | --- |
| `kind` | `AiJobKind` — what the job produces. `text`, `theme`, `layout`, `template`, `form` and `site` have runners; every other kind fails fast with "not available yet" until its own issue lands. |
