...
| A property condition's `pattern` (AGL-2893) | A condition rule may test a property's value against a regular expression, written by whoever declared the property — a marketplace publisher, a template, a document written without the Properties dialog. `new RegExp` on it threw for an invalid pattern or flags, and the tenant loader's catch turned that into a 404 for every page placing the component; a valid catastrophic pattern (`^(a+)+$`) backtracked for seconds on a 28-character default, doubling per character, on the shared render path and in every installer's console. | **Fixed:** patterns are matched by the linear-time engine (`compileLinearTest`, `libs/shared/util/tools/src/lib/linear-regex.ts`, moved from the redirects plugin), never by `RegExp`, so every accepted pattern costs `O(value × program)`. `reusablePropPatternProblem` (`libs/aglyn/src/lib/app-utils/reusable-prop-values.ts`) refuses a pattern over 500 characters, flags `RegExp` would refuse or the engine lacks (`u`, `v`), syntax outside the engine's subset, and anything `RegExp` refuses; a refused pattern, or a value over 2,000 characters, holds nothing at render and never throws. The Properties dialog shows the same refusal, and marketplace publish/install/update, `/api/hosts/resources`, `/api/hosts/versions` and site import drop a rule they refuse (`withMatchableConditions`). Pinned in `reusable-prop-values.spec.ts`, `linear-regex.spec.ts` (differential against `RegExp`), `marketplace-props.spec.ts`, `host-resources-allow-list.spec.ts`, `import-resource-caps.spec.ts` and `apps/tenant/specs/component-property-values-render.spec.ts`. |
