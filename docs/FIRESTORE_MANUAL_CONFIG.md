...
| `aiJobs` | `expiresAt` | An AI generation job (AGL-2904): the customer's **brief verbatim**, the step ledger, the outputs it named and the creating `uid`, under `orgs/{orgId}`. **180 days**, the exchange's clock — `createAiJob` in `libs/tenant/data/admin/src/lib/server/ai-jobs.ts` stamps `assistExchangeExpiry(now)`. The drafts a job writes are ordinary content and are not touched by this policy. **TTL OWED**: declared and written on 2026-09-14; the gcloud command below has not been run. |
| `months` | `expiresAt` | A person's monthly AI usage (AGL-2928), one document per uid per month at `orgs/{orgId}/aiUsageByUser/{uid}/months/{YYYY-MM}`: credits, spend, request and refusal counts, a split by kind and by site. Integers keyed by a uid, no prose. **Thirteen months** past the month the document describes (`AI_USAGE_BY_USER_RETENTION_MONTHS` in `libs/aglyn/src/lib/app-utils/ai-usage-by-user.ts`) — a year of month-over-month comparison for a workspace admin, plus the month in progress. The collection-group name is the generic `months`; nothing else in the schema writes a subcollection by that name, and the `uid` field carries a collection-group index beside this so an account erasure can sweep a person's months out of workspaces they had already left. **TTL OWED**: declared and written on 2026-09-14; the gcloud command below has not been run. |
...
gcloud firestore fields ttls update expiresAt \
  ...
  --project=aglyn-main --database='(default)'
# AGL-2904 — OWED, not yet run:
gcloud firestore fields ttls update expiresAt \
  --collection-group=aiJobs --enable-ttl \
  --project=aglyn-main --database='(default)'
# AGL-2928 — OWED, not yet run:
gcloud firestore fields ttls update expiresAt \
...
- TTL policies: **eight `ACTIVE`, two owed** (`aiJobs`, AGL-2904, and `months`, AGL-2928, both declared 2026-09-14 — run their commands above) — active: `rateLimits` (AGL-870), `mediaTombstones` (AGL-1467), `cspViolationDaily` (AGL-1799), `analytics` and `screenAnalytics` (AGL-1844), plus `assistExchanges` (AGL-1972), `churnSurveyDetails` and `apiIdempotency` (AGL-1978) enabled 2026-08-19, all eight re-verified 2026-08-19 with `gcloud firestore fields ttls list --project=aglyn-main --database='(default)'`. The retention schedule they implement is [`docs/DATA_RETENTION.md`](DATA_RETENTION.md); `apps/console/specs/retention-ttl-config.spec.ts` fails the build if a declaration, a doc row or a writer goes missing — it cannot see gcloud, which is why the owed state is written down here
