...
can't. The one checked-in example is the **sandbox marketplace plugin**
...
| Marketplace realm bundle | `tools/plugin-loader/realm/demo` (+ `EXPLAINED.md`) | The standalone track, narrated line by line |
| Marketplace starter | `tools/plugin-loader/realm/template` | What you actually copy to begin |
| Sandbox marketplace plugin | `examples/plugins/promo-countdown` | A complete, publishable sandbox plugin with no build step — `register()`, render, props, config scheme, emitted events, and cleanup. Passes the bundle verifier as-is, so it doubles as the worked example for [uploading a plugin](../publishing/publisher-handbook.md) |
