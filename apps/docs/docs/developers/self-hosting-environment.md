...
| `AGLYN_TENANT_SHARED_POOL_SIZE` | Optional | Runtime | How many **shared** sending domains this deployment runs — `shared1.{mail apex}`, `shared2.…` and so on — for sites that have no domain of their own. A whole number 1–64; default `4`, and a blank, unparsable or sub-1 value falls back to it. The pool is fixed-size and does not grow with sites, so its whole cost is this many provider domain objects and three zone records each, whether you host twelve sites or a hundred thousand. Four rather than one because every site on a member shares its reputation: one member in trouble takes a quarter of your transactional mail with it, not all of it. Raising it is a real decision — each member is a reputation that has to be earned by sending steadily, and each spends a provider domain slot counted against `AGLYN_SENDING_DOMAIN_CAPACITY`. Changing it does **not** shuffle existing sites: assignment is rendezvous-hashed, so growing the pool moves only about `1/size` of them onto the new member and nothing else observes the change. Every pool label is reserved against tenants at any size, so a site can never be handed a name the pool is about to want. Unset, you get four; there is no configuration that produces no pool, and a site with no domain of its own would then have no sending identity at all. |
| `AGLYN_TENANT_SHARED_LOCAL_PART` | Optional | Runtime | The mailbox shared-pool mail is sent from — the part before the `@`. Default `notifications`, giving `notifications@shared2.{mail apex}`. Normalized like any local part, and a value that normalizes to nothing falls back to the default. Deliberately not `hello`, which is what a site's **own** domain defaults to: the two are different promises, and an address inviting replies to a mailbox belonging to no particular site invites mail nobody reads. Only the local part is configurable — the address always sits directly **on** a pool member, never beneath one, because under strict DMARC alignment the `From:` domain and the DKIM `d=` must be the same name and each member signs only for itself. |
