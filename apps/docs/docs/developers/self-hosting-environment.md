...
| `AGLYN_FUNNEL_MIN_LEAD_FORMS` | Optional *(scripts only)* | Runtime | Floor on how many lead-routing forms `/api/health/funnel` expects to find before it grades them. A deployment with fewer legitimately has fewer; raise or lower it to match, rather than letting a missing form read as healthy. |
