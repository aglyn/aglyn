...
saying how many are selected, with one action for all of them. Each section's
bar offers what that record can do; every bar has **Export CSV** and **Clear**.
...
interaction, notes, and one column per [custom field](./custom-fields.md),
...
from the header alone, except **Sources** and **Last interaction**, which the
platform records and a file cannot set. **Download template** in the Import
