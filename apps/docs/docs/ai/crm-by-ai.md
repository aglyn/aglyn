...
record's id.
When an email address or a phone number is typed into a note or a logged activity, it is
replaced before the text is sent. A street address typed into a note is not recognized, so it is
sent as typed.
