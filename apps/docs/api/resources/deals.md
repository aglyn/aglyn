...
| `nextTaskAt` | string \| null | When the earliest **open** [task](tasks.md) filed against the deal is due, or `null` when none is — what the console's "stuck deals" figure counts. Maintained by every task write. **Read-only.** |
