...
const DECLARED: Readonly<Record<string, DeclaredSinkFile>> = {
  ...
  'libs/aglyn/src/lib/app-utils/docs-index.generated.ts': {
    markers: 2,
    guard: 'platform',
    why: "Not a sink: the docs search index (AGL-2939), generated by tools/scripts/generate-docs-help.mjs from apps/docs — our own help pages' headings and excerpts, two of which happen to read `image: none` inside a sentence about CSS. No author string reaches it and no URL is produced here; it is read by the console's issue-report deflection and the assistant's grounding, never on a tenant page.",
  },
  'libs/plugins/ai/src/lib/jobs/ai-job-products-step.ts': {
    markers: 1,
    guard: 'off-tenant',
    why: "Not a sink, and never on a tenant render: the products job step (AGL-2916) runs on the console's AI jobs beat and its inline doors. `product.imageUrl` is handed to `readAiProductImage`, which reads the photo's bytes server-side for the model; no URL is rendered for a visitor.",
  },
  'libs/plugins/ai/src/lib/jobs/ai-product-image.ts': {
    markers: 1,
    guard: 'off-tenant',
    why: "Not a sink, and never on a tenant render: the products job's photo reader (AGL-2916), called only by that job step on the console. It resolves the product's first media item to an asset of the site's or org's own media library and returns re-encoded bytes for the model, never a URL for a page.",
  },
  'libs/plugins/ai/src/lib/runtime/ai-doctrine-validators.ts': {
    markers: 1,
    guard: 'off-tenant',
  ...
  'libs/plugins/ai/src/lib/runtime/ai-palette.generated.ts': {
    markers: 1,
    guard: 'platform',
    why: "Not a sink: the AI palette (AGL-2905), generated by tools/scripts/generate-ai-palette.mts from the component bundles — the JSON SCHEMA that DESCRIBES each component's props, and the marker is the Social Links element's `twitter` property key. It is handed to the model as a prompt; nothing in it is rendered, and no author string reaches it.",
  },
  'libs/plugins/ai/src/lib/jobs/ai-job-seo-step.ts': {
    markers: 2,
    guard: 'off-tenant',
  ...
  'libs/plugins/ai/src/lib/runtime/seo-fields.ts': {
    markers: 1,
    guard: 'off-tenant',
    why: "Not a sink, and never on a tenant render: SEO by AI's listing request (AGL-2910), whose `generateSeoFields` the `seo` job step (jobs/ai-job-seo-step.ts) calls for a job on the console's AI job doors and from the job beat. The marker is the `Share image:` line of the prompt `aiSeoFieldsPrompt` writes for the model: the image's current description and the page text beside it, as words, never its URL. Nothing here renders a page or emits a URL; what the model returns is a proposal, applied only when a member saves it in the console.",
  },
