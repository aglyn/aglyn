...
/**
 * The tenant deployment's `/api/health` can go red (AGL-2592).
 *
 * The door an external monitor reads to learn whether the site runtime is
 * serving at all. Its one check is a Firestore read of a document that is
 * meant to be missing — a missing document proves credentials, network and
 * the API all work, and a thrown read is the outage the door exists to
 * report. Until this file existed nothing asserted it could answer anything
 * but 200.
 *
...
describe('Firestore answers the probe read', () => {
  it('answers 200 and carries the monitor keyword exactly once', async () => {
    ...
    expect(seen.body['service']).toBe('tenant')
  ...
  it('reads a document that is meant to be missing, and nothing else', async () => {
    await probe()
    expect(mockReads).toEqual(['orgSlugs/tenant-health-probe-does-not-exist'])
  })
