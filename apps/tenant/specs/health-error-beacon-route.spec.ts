...
/**
 * The tenant deployment's `/api/health/error-beacon` can go red, and stops
 * going red for nothing (AGL-2592, AGL-2713).
 *
 * The beacon heartbeat is written by two deployments with two different admin
 * credentials, so the console and the tenant each carry their own door. The
 * console's door got the sibling spec in AGL-1923; this is the tenant half,
 * held to the same contract. The external monitor on this route is a keyword
 * check for `"status":"ok"`, alerting when the keyword is ABSENT, which only
 * means something if the route stops saying `ok` when the heartbeat cannot be
 * written.
 *
...
// program `tsc` builds over `apps/tenant`, so the export keeps it a module.
export {}
...
describe('the beacon heartbeat lands', () => {
  ...
  it('names the log and the TENANT service an operator would query', async () => {
    const seen = await probe()
    expect(seen.beacon['logId']).toBe('client-error-beacon-heartbeat')
    expect(seen.beacon['service']).toBe('tenant-web')
    expect(seen.body['service']).toBe('tenant-error-beacon')
  })
