 ...
 * names for the `tenantApi` surface. The plugins that ask are specced against a
...
async function registerEverySurface(): Promise<string[]> {
  const ran: string[] = []
  for (const entry of TENANT_PLUGIN_SERVER_MANIFEST) {
    const registrar = (entry.register as Record<string, string | undefined>)['tenantApi']
