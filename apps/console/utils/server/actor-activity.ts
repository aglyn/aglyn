...
/**
 * The scopes a person ADMINISTERS: the organizations where they hold owner or
 * admin, and every site those organizations own.
 *
 * Read from the per-user projection `users/{uid}/orgs`, which the membership
 * APIs maintain, rather than by scanning every org's roster — the same
 * projection the console routes a signed-in user with.
 *
 * "Administers" is deliberately narrower than "belongs to". This set decides
 * what appears under a heading on a STAFF page about one person, and an editor
 * on somebody else's site has no administrative relationship to that site's
 * activity; listing it there would make a staff reader think they were looking
 * at that person's remit.
 */
export async function administeredScopePaths(
  uid: string,
): Promise<Set<string>> {
  ...
  for (const membership of memberships.docs) {
    ...
    for (const path of await orgActivityScopePaths(membership.id)) {
      paths.add(path)
    }
  }
  return paths
...
/**
 * Activity on the scopes a person administers whose ACTOR WAS NOT RECORDED.
 *
 * The staff user page asks "what did this account do" and answers it with
 * `actorId == uid`, which is the only honest answer to that question. But
 * every artifact under a host was created without an author field until
 * AGL-118, and three template surfaces logged nothing at all, so for a person
 * who built a site from a template that query returns zero and the page reads
 * as if they had never used the product.
 *
 * This is the other half, and it is a DIFFERENT question — "what happened on
 * the workspaces this person is responsible for, that nobody's name is
 * attached to" — which is why it is a separate reader feeding a separate
 * section rather than a widening of the one above. Merging them would put
 * rows nothing attributes to this person under a heading that says they did
 * them, which is the failure the whole exercise exists to avoid: a
 * reconstructed row is evidence about a SITE, never about a person.
 *
 * `actorId == null` is an equality on a written null, not a missing-field
 * test: the client logger writes `actorId: user?.uid ?? null` and the
 * reconstruction writes an explicit null, so both are matched and neither is
 * inferred. It reuses the existing collection-group index on
 * (`actorId`, `createdAt`) — the same one the attributed query uses — so this
 * costs no index deploy.
 *
 * `scopePaths` is REQUIRED here, unlike above where absent means "every
 * scope, the staff view". There is no sensible unscoped form of this query: an
 * unattributed row belongs to whoever administers its site, and returning
 * every unattributed row on the platform under one person's name is precisely
 * the leak this is scoped to prevent.
 */
export async function readUnattributedScopeActivity(options: {
  uid: string
  pageSize: number
  cursor?: string | null
  filter?: ListFilterInput | null
}): Promise<ActorActivityPage> {
  const scopePaths = await administeredScopePaths(options.uid)
  // A person who administers nothing has an empty section, not the platform's.
  if (!scopePaths.size) return { entries: [], nextCursor: null, scanned: 0 }
  ...
  let after = options.cursor
    ? await firestore.doc(options.cursor).get().catch(() => null)
    : null
  if (after && !after.exists) after = null
  ...
  while (entries.length < pageSize && scanned < SCAN_CAP && !exhausted) {
    ...
    if (snapshot.docs.length < batchSize) exhausted = true
    for (const doc of snapshot.docs) {
      // Same off-by-one discipline as the attributed reader: never advance the
      // resume point past a row the page had no room to show.
      if (entries.length >= pageSize) break
      scanned += 1
      after = doc
      const scopePath = doc.ref.parent.parent?.path ?? ''
      if (!scopePaths.has(scopePath)) continue
      const data = doc.data() as Record<string, unknown>
      const seconds = timestampSeconds(data['createdAt'])
      entries.push({
        $id: doc.id,
        ...
        action: typeof data['action'] === 'string' ? data['action'] : undefined,
        target: (data['target'] as Record<string, unknown> | null) ?? null,
        // Null by construction — the row has no actor. Carried explicitly so
        // the table can render "not recorded" rather than an empty cell, which
        // reads as a rendering bug and invites the reader to assume the actor
        // is simply the person whose page this is.
        actorEmail: null,
        createdAt: seconds === null ? null : { seconds },
      })
    }
