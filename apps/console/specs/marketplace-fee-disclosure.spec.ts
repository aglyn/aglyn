...
/*
 * Read from the MARKETPLACE PLUGIN, where the panel lives since AGL-3080.
 *
 * The pin stays in this app rather than following the panel, because what it
 * needs is `source-text`'s stripper — the one comment stripper the source-guard
 * specs share (AGL-1479), whose whole reason for existing is that four
 * hand-rolled copies silently deleted most of their input. A fifth copy inside
 * the plugin to avoid an app→lib path would be that defect, chosen on purpose.
 */
