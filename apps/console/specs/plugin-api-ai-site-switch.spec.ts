 ...
 * progress or an insight job's answer, and canceling a job, stay open, so a
 * job the switch failed can still be read and a queued one stopped. The beat
 * fails the job itself.
 */
const JOB_DOORS = ['ai/jobs/:jobId/cancel', 'ai/jobs/:jobId/events', 'ai/insights/:jobId']
...
describe('the workspace half keeps running whatever a site decided', () => {
  ...
  it.each(JOB_DOORS)('%s runs: it names a job, not a site', async (path) => {
    const method = path.endsWith('/events') || path.startsWith('ai/insights/') ? 'GET' : 'POST'
    const response = await call(path, method, null)
    expect(response.status).toBe(200)
    expect(mockCalls).toEqual([path])
  })
