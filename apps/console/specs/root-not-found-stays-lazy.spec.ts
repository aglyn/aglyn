...
const SCREEN = 'components/root-not-found.component.tsx'
...
describe('the console chrome stays out of the root not-found chunk group', () => {
  it('reaches the screen only through a dynamic import', () => {
    ...
    expect(source).toMatch(
      /dynamic\(\s*\(\)\s*=>\s*import\(\s*'\.\.\/components\/root-not-found\.component'\s*\)/,
    )
    expect(staticValueImportSpecifiers(source)).not.toContain(
      '../components/root-not-found.component',
    )
  })
  ...
  it('gives the lazy screen a Suspense boundary of its own', () => {
    /**
     * `next/dynamic` supplies its own `<Suspense>` only when the call passes
     * `ssr: false` or an explicit `loading` — otherwise it renders a Fragment
     * and leans on whatever ancestor boundary happens to be above it, which
     * is the AGL-1541 shape. This boundary renders in the served markup, so
     * `ssr` stays true and `loading` is what earns the boundary.
     */
    expect(read(BOUNDARY)).toMatch(/loading:\s*\(\)\s*=>\s*null/)
  })
  ...
  it('composes the same three pieces it used to compose inline', () => {
    ...
    const specifiers = staticValueImportSpecifiers(read(SCREEN))
