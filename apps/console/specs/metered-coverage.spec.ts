...
const EXEMPT: Record<string, string> = {
  ...
  'app/api/billing/profile/route.ts':
    'The native billing settings surface touches a subscription in exactly one place: `subscriptions/{id}` with `default_payment_method` and nothing else, so that changing the default card actually changes the card the next renewal charges — a customer default alone does not, because Checkout writes the method onto the SUBSCRIPTION (AGL-940). Same shape as `admin/org-discount` and `billing/retention` above: it never sends `items`, so the metered item is neither added nor dropped, and it cannot change the plan or the interval that would decide which metered price applies. Its update emits customer.subscription.updated, so the webhook back-fill covers the subscription anyway. Everything else in the route reads Stripe or writes the CUSTOMER (email, address, tax ids, payment methods); the one session it creates is `mode: setup`, which attaches a card and has no line items at all.',
