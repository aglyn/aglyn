...
const AI_DOORS = new Map<string, string>([
  ...
  [
    'libs/plugins/ai/src/lib/jobs/ai-job-experiment-step.ts',
    'A generation job\u2019s experiment step (AGL-2914), the A/B door. Two shapes, and it writes nothing either way. FOR VARIANTS: the brief, what the test varies, and the copy under test as the A/B testing card handed it over \u2014 site copy of the surface being tested, the same class the besigner copy assistant and the page step already send. It answers between two and four variants of that copy and NOTHING else \u2014 no weight, no traffic split, no schedule \u2014 and the answer fills the editor\u2019s own unsaved fields, where the card\u2019s Save is the only write: the experiment document, its variants\u2019 screen versions and its traffic stay marketing\u2019s, so no job can start a test on live visitors. FOR A RESULT: the test\u2019s name, and each arm as `id, exposures, conversions, rate` exactly as the figure reader published it, plus the verdict \u2014 which is code\u2019s own sentence, decided by `readAiExperimentResult` from those counts BEFORE the model is asked, and stated in the request so the answer can be cut back to it. NO VISITOR, NO SESSION AND NO ASSIGNMENT IS READ, because the reader publishes none: it answers in aggregates through core\u2019s `plugin-figures` seam, which is the whole reason a figure reader is what this reads. The AI plugin never imports marketing. Behind `release_ai_generative`, the `aiGenerative` entitlement, the `ai-generate` lockdown key and the `ai.generate` permission \u2014 and additionally behind the `abTesting` plan entitlement, which `plan-entitlements.ts` gives from Business up, so it runs on no Free, Starter or Pro workspace at all. THE PUBLISHED ANTHROPIC ROW ALREADY NAMES BOTH CATEGORIES, in the row published September 17, 2026 (legal v8, AGL-3069): its `dataReceived` names \u201cA/B test and variant conversions\u201d among the aggregate tables, and the copy under test is \u201cthe content of the element, post, section or page being worked on\u201d. No new KIND of data reaches Anthropic here. What the published row does NOT do is name variant-writing as its own feature: its purpose list reaches A/B tests under \u201cAI insights, which answer a customer\u2019s questions about its own figures\u201d, and reaches this door only under the general \u201cAI generation, which creates drafts and proposals \u2026 such as copy\u201d. That is accurate but oblique, and the next legal revision should name it plainly rather than leave a reader to infer it. The flag stays off for customers meanwhile, so nothing has flowed.',
  ],
  [
  ...
  [
    'libs/plugins/ai/src/lib/server/plugin-text-generation.ts',
    'The AI plugin as the workspace’s text generator on the core’s `plugin-text-generation` seam (AGL-3324): another plugin in the process asks for a paragraph without importing this one, and the seam sends what THAT caller hands it — its standing instructions as the system block and its prompt as the one user turn — after climbing the same rungs a generation job climbs: `release_ai_generative`, the `aiGenerative` entitlement, the caller’s `ai.generate` on the site named, the `ai-generate` lockdown key, the per-site AI switch, and a reservation against the org’s and the member’s allotments; metered under the purpose the caller names. ONE CALLER TODAY, Sequences’ curation (`libs/plugins/outreach/src/lib/routes/curate-routes.ts`, purpose `outreach-curate`), which sends: the sequence’s own email steps as their author wrote them (subject and body, merge tokens included — site content the workspace wrote); the sender’s display name and the site’s name; the rep’s personal line for this person; and what the CRM holds about the ONE person being written to, as `enroll-people.ts` reads them for enrolling — their name, company, job title, the company’s website or domain, tags, how they came to the workspace (the capture kinds), the names of the campaigns they are filed under, whether they are a lead or a contact, and for a lead its notes cut to 600 characters. NO email address, phone number, postal address, order, booking, consent record or other person is sent: the address is what the email is sent to and never enters the prompt, and an address or a number typed into the notes or the personal line is replaced by `[email address]` or `[phone number]` before the text leaves (`engine/curation-prompt.ts`). The answer is a draft the member reads, edits and confirms; it is stored on the enrollment only then, and the seam writes nothing. THE PUBLISHED ANTHROPIC ROW ALREADY NAMES THE CATEGORIES: the row and the Privacy Policy published on September 17, 2026 (legal v8, AGL-3069) name CRM records — a contact’s or a lead’s name, title, company, tags, capture kinds and notes with addresses and numbers replaced, the same fields and the same placeholders the `crm` step sends — and email requests, and the email steps are the workspace’s own site content of the kind the copy assistant already sends. No new KIND of data reaches Anthropic here. What the published row does NOT do is name a sales sequence’s per-person drafting as its own feature: it reaches this door under “AI generation, which creates drafts and proposals … such as copy” and under CRM records, which is accurate but oblique, and the next legal revision should say it plainly. The flag stays off for customers meanwhile, so nothing has flowed.',
  ],
])
