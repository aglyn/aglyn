...
export async function generateMetadata({
  ...
  params: Promise<{ host: string; templateId: string }>
}): Promise<Metadata> {
  const { host, templateId } = await params
  return {
    title: entityPageTitle({ subject: templateId, noun: 'Template preview', scope: host }),
  }
}

export default function TemplatePreviewTitleLayout({ children }: { children: ReactNode }) {
  return <>{children}</>
}
