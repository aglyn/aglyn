...
import { readUnattributedScopeActivity } from '../../../../utils/server/actor-activity'
import { readListFilter } from '../../../../utils/server/list-filter'

/**
 * Activity on the workspaces and sites this account ADMINISTERS whose actor
 * was never recorded (AGL-118).
 *
 * The sibling route `/api/admin/user-activity` answers "what did this account
 * do" with `actorId == uid`. This one answers a different question and must
 * never be mistaken for it: these rows are attributed to NOBODY, and the only
 * thing linking them to the account whose page they appear on is that the
 * account administers the site they happened on.
 *
 * Separate route, separate reader, separate section — so that no widening of
 * one can quietly fold unattributed rows into the attributed list.
 */
...
async function handler(request: Request): Promise<Response> {
  ...
  try {
    ...
    const page = await readUnattributedScopeActivity({
      uid,
