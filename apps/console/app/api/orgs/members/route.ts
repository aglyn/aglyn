...
import {
  ...
  notifyUsers,
  removeOrgMember,
  resolveMemberPluginPermissionsOnOrg,
...
async function handler(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (action === 'upsert') {
      ...
      // The plugin-declared verdicts before the write, for an existing member
      // only: a role or a custom-role assignment can move a key a plugin
      // declared into the catalog, and each key that moved is one platform
      // event (AGL-2929). A person added just now had no verdict to move;
      // the add is its own row. `null` for a collaborator, whose per-site
      // keys are decided per site.
      const pluginBefore = existedAlready
        ? await resolveMemberPluginPermissionsOnOrg(orgId, targetUid)
        : null
      ...
      if (pluginBefore) {
        ...
        const pluginAfter = await resolveMemberPluginPermissionsOnOrg(orgId, targetUid)
        for (const change of pluginPermissionChanges(pluginBefore, pluginAfter)) {
