 ...
 *
 * - the HOST DOCUMENT, one subscription feeding every form in the hub. A copy
 *   per section would open a second listener and re-seed the forms on every
 *   rail click.
...
export function useHostSettingsScope(): HostSettingsScope {
  const scope = useContext(HostSettingsScopeContext)
  if (!scope) {
    throw new Error(
      'useHostSettingsScope must be used inside a settings sections layout',
    )
  }
  return scope
}
...
/**
 * One host settings form, by schema id.
 *
...
/**
 * The host document, the guarded saves and the drafts, for BOTH settings hubs.
 *
 * The forms this scope serves are split across two hubs: Basic details is a
 * question about the site as an object — its address and its name in the
 * console — and lives under Admin, while SEO and Tracking describe how the
 * published site behaves and stay under Setup. One save path serves all three.
 *
 * A module rather than a copy per hub, and the reason is `handleBasicSave`: it
 * carries the unconfirmed-seed guard (AGL-1358), and a second copy of a
 * guarded write is how the two come to drift and one of them silently stops
 * guarding. The listener, the drafts and the guard are one mechanism, so they
 * are lifted whole and each hub's sections layout mounts the same one.
 *
 * Mounted by a LAYOUT, never by a page. The drafts ref is the reason: sections
 * are routes, so the page being left is unmounted, and a half-typed edit only
 * survives the move because the ref sits above the route (AGL-2501).
 */
export function HostSettingsScopeProvider({
  children,
}: {
  children: ReactNode
}) {
    ...
    async (
      ...
      await setDoc(clientFields, { merge: true })
        .then(() => {
          ...
          //
          // To the General section specifically: `basicSchema` is the only
          // form carrying a `subdomain` field, so it is the only save that can
          // reach here, and landing the author back on the card they just
          // saved is what the old URL was showing them.
          if (renamedTo && renamedTo !== host) {
            router.replace(
              buildRoute(Route.HOST_ADMIN_GENERAL, {
                orgSlug,
                host: renamedTo,
              }),
            )
  ...
  return (
    <HostSettingsScopeContext.Provider value={scope}>
      {children}
    </HostSettingsScopeContext.Provider>
  )
}

HostSettingsScopeProvider.displayName = 'HostSettingsScopeProvider'
