...
import { mdiCogOutline } from '@aglyn/shared-data-mdi'
...
type Scope =
  | { kind: 'account' }
  | { kind: 'org' | 'host'; id: string; label: string }
...
> = () => {
   ...
   */
  const { loading: orgsLoading } = useOrgScope()
  ...
  /*
   * The digest keys are ARGUMENTS to a core helper, never declarations here
   * (`check:plugin-domain-in-core`, rule 7): a console page may call a
   * plugin's seam and must not spell its vocabulary into a name or an object
   * member of its own. Written inline for that reason, which is also how the
   * feed page carried it before AGL-3226 moved it.
   *
   * The card itself is still core naming a plugin's schedule, which is the
   * AGL-3227 seam — this page will render whatever digests are registered
   * rather than the two it knows about.
   */
  const toggleDigest = () => {
    if (!uid) return
  ...
  return (
    <DashboardLayout
      breadcrumbItems={[
        { children: 'Notifications', href: buildRoute(Route.MANAGE_NOTIFICATIONS) },
        {
          children: 'Settings',
      ...
      help={{ topic: 'consoleTour', anchor: '#notification-settings' }}
    >
                              ...
                              <ToggleButtonGroup
                                ...
                                onChange={(_event, next) => {
                                  ...
                                  write(
                                    ...
                                    category,
                                    channel.key,
                                    next === 'inherit' ? undefined : next === 'true',
                                  )
                                }}
                                aria-label={`${label} — ${channel.label}`}
                              >
                    ...
                    .map((orgId) => (
                      <FormControlLabel
                        ...
                        control={
                          <Switch
                            size="small"
                            checked={insightDigestSubscribed(
                              insightDigests,
                              orgId,
                            )}
